<template>
  ...
  <div class="d-flex ">
  <div >
    <input class="form-control me-5 " id="password" v-model="password" :type="showPassword ? 'text' : 'password'" placeholder="Enter a password">
    ...
    <div class="">
      <button id="showPasswordButton" class="btn btn-outline-secondary" type="button" @click="togglePasswordVisibility">
        ...
        <button class="btn btn-bd-primary w-100 mt-4 shadow p-2 mb-2" v-on:click.prevent=" login">Log In</button> <!-- Added emailClicked and passwordClicked variables -->
...
export default {
  ...
  methods: {
    ...
    async login() {
      ...
      try{
      let result = await axios.post ("http://172.16.4.18:8000/api/login",{
          email: this.email,
